...
| `nanobanana` | `image`, `images[]` | `size` -> aspect ratio; async polling |
| `fal-ai` | `image` | `n` -> `num_images`; `size` -> ratio |
| `stability-ai` | `style`, `output_format` | `size` -> `aspect_ratio` |
