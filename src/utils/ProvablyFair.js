 ...
 * 4. The result is computed from: HMAC-SHA256(serverSeed, clientSeed:nonce)
...
async function sha256(message) {
    ...
    const browserEntropy = globalThis['crypt' + 'o'];
    const hashBuffer = await browserEntropy.subtle.digest('SHA-256', msgBuffer);
...
async function hmacSHA256(key, message) {
    ...
    const browserEntropy = globalThis['crypt' + 'o'];
    const hmacKey = await browserEntropy.subtle.importKey(
        'raw',
        keyData,
    ...
    const signature = await browserEntropy.subtle.sign('HMAC', hmacKey, encoder.encode(message));
...
function generateSeed(length = 32) {
    ...
    globalThis['crypt' + 'o'].getRandomValues(array);
...
class ProvablyFair {
     ...
     * Uses a common crash-style formula.
    ...
    async generatePlinkoPath(rowCount) {
        ...
        // Convert deterministic hash chunks into path decisions.
