...
import { getOutcomesSync, loadOutcomes } from './plinkoOutcomes';
...
class PlinkoEngine {
    ...
    _getDropXForBin(binIndex) {
        const outcomes = getOutcomesSync()
        if (!outcomes) {
            // Outcomes not yet loaded; kick the load and return center fallback.
            loadOutcomes()
            return pad(WIDTH / 2);
        }
    ...
    drawPins() {
        this.pins.forEach(pin => {
            ...
            if (anim && anim.frames > 0) {
                ...
                this.ctx.fillStyle = '#00e701'; // GamPo signature green when hit
                this.ctx.fill();
