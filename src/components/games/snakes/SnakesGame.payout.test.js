...
const source = readFileSync(new URL('./SnakesGame.jsx', import.meta.url), 'utf8')
...
const MULTIPLIERS = [1.09,1.2396694214876034,1.45,2.13,3.7]
const STAKES = [1, 2, 3, 5, 7, 10, 25, 100, 137]
const TWO_DP = /^-?\d+(\.\d{1,2})?$/

describe('snakes payout rounding', () => {
...
describe('snakes is pinned to the component', () => {
    it('rounds the return and the profit', () => {
        ...
        expect(source).toContain("addWinnings(totalReturn, 'Snakes return')")
    })
...
describe('snakes component parses', () => {
    ...
    it('imports without a syntax or module-resolution error', async () => {
        const mod = await import('./SnakesGame.jsx')
