...
const source = readFileSync(new URL('./DrillGame.jsx', import.meta.url), 'utf8')
...
const MULTIPLIERS = [1.17,1.43,1.74,2.12,2.59,3.16,3.85,4.7]
const STAKES = [1, 2, 3, 5, 7, 10, 25, 100, 137]
const TWO_DP = /^-?\d+(\.\d{1,2})?$/

describe('drill payout rounding', () => {
...
describe('drill is pinned to the component', () => {
    it('rounds the return and the profit', () => {
        ...
        expect(source).toContain("addWinnings(totalReturn, 'Drill return')")
    })
...
describe('drill component parses', () => {
    ...
    it('imports without a syntax or module-resolution error', async () => {
        const mod = await import('./DrillGame.jsx')
