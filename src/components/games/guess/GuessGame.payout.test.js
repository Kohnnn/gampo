...
const source = readFileSync(new URL('./GuessGame.jsx', import.meta.url), 'utf8')
...
const MULTIPLIERS = [9.4]
const STAKES = [1, 2, 3, 5, 7, 10, 25, 100, 137]
const TWO_DP = /^-?\d+(\.\d{1,2})?$/

describe('guess number payout rounding', () => {
...
describe('guess number is pinned to the component', () => {
    it('rounds the return and the profit', () => {
        ...
        expect(source).toContain("const payout = 9.4")
    })
...
describe('guess number component parses', () => {
    ...
    it('imports without a syntax or module-resolution error', async () => {
        const mod = await import('./GuessGame.jsx')
