...
const source = readFileSync(new URL('./ColorGame.jsx', import.meta.url), 'utf8')
...
const MULTIPLIERS = [3.84]
const STAKES = [1, 2, 3, 5, 7, 10, 25, 100, 137]
const TWO_DP = /^-?\d+(\.\d{1,2})?$/

describe('color pick payout rounding', () => {
...
describe('color pick is pinned to the component', () => {
    it('rounds the return and the profit', () => {
        ...
        expect(source).toContain("const payout = 3.84")
    })
...
describe('color pick component parses', () => {
    ...
    it('imports without a syntax or module-resolution error', async () => {
        const mod = await import('./ColorGame.jsx')
