...
describe('poker layout CSS', () => {
    ...
    it('keeps mobile poker entry and action controls reachable', () => {
        ...
        expect(css).toMatch(/\.pk-seat\.is-human \.pk-seat-cards \.pk-card\s*\{[\s\S]*width:\s*clamp\(46px, 14vw, 54px\)/s)
        expect(css).toMatch(/\.pk-seat:not\(\.is-human\) \.pk-seat-cards \.pk-card\s*\{[\s\S]*width:\s*22px/s)
        expect(css).toMatch(/\.pk-seat:not\(\.is-human\) \.pk-seat-cards \.pk-pip\s*\{\s*font-size:\s*14px/s)
        expect(css).toMatch(/\.pk-seat:not\(\.is-human\) \.pk-card-back::after\s*\{\s*font-size:\s*12px/s)
        expect(css).toMatch(/\.pk-actions\.is-waiting:not\(\.is-showdown\)\s*\{[\s\S]*max-height:\s*72px/s)
