...
- If the plan carries an `execution-authority-envelope/v1` block, it passes its validator

---

## Execution Authority Envelope (consumed by EXECUTE)

VALIDATE produces the envelope (see [08-validate.md](08-validate.md)); EXECUTE consumes it.
When the selected plan's `## Validate Contract` carries an `execution-authority-envelope/v1`
block, run:
...
at the cadence selected by the envelope branch:
...
2. Only the legacy Report/Correction lane reruns at the mid-phase checkpoint. Report mode may update its schema-bound Report; correction mode writes no generic progress note and defers its one immutable creation until completion.
3. Only the legacy Report/Correction lane reruns before writing the completion artifact or emitting the exit summary, confirms the output path equals `artifact_path`, and preserves the schema's write semantics. The temporary lane instead performs its per-target postreadback checks and does not rerun the full envelope.
...
A standing `/goal` grant, autopilot mode, or prior PASS does not override a failing envelope.

**Scope enforcement is an agent contract, not a filesystem guard.** Before each write,
normalize the target path (`\` → `/`) and confirm an `allowed_scope` entry covers it (exact
match or `dir/**` subtree prefix). No interception layer exists — an out-of-scope write is a
contract violation the agent is responsible for preventing, and the validator cannot detect
it after the fact.

A passing envelope authorizes only reversible local work inside `allowed_scope`. Every
`stop_conditions` entry and every standing hard stop remains authoritative.

Artifact consumption is branch- and schema-dependent:

- With `authorityClass` absent, preserve all three legacy preflight points and behavior. `phase-report/v1` uses the exact `**Report destination:**`; `phase-report-correction/v1` uses the exact `**Correction destination:**` under [10-update-process.md](10-update-process.md), leaving the original Report immutable.
- With `authorityClass: "temporary-artifact-set/v1"`, run exactly one initial envelope preflight and no mid/completion full-envelope reruns. For each of the 1–8 exact targets, immediately before creation recheck lexical exactness and DOS-device rejection, absence, the lexical approved-root chain by `lstat` before root `realpath`, frozen root identity, component-wise ancestry `lstat`, and realpath containment; create with `wx`; flush/close; reopen and rerun the complete root-chain/root-identity/ancestry/containment sequence plus destination `lstat` regular/non-reparse and destination realpath checks before verifying bytes, schema marker, and SHA-256. Stop on collision, partial set, root/ancestry/destination drift, alias/reparse, mismatch, or unauthorized target. Do not rerun the full envelope per target.
- Emit one in-memory/evidence receipt per successful target with ordered `schema,artifactPath,artifactSchemaVersion,bytes,sha256,exclusiveCreate,regularNonReparse,readbackMatches,status` keys. A receipt is not a fifth artifact and is never written beside targets.
- Use the validator's read-only injected observation seam for deterministic reparse rejection. Attempt the disposable Windows junction probe when policy permits; record inability as `SKIP`, never PASS. Node `realpath`/`lstat` does not prove every reparse tag or eliminate TOCTOU risk.
- With `authorityClass: "repository-diagnostic-evidence-set/v2"`, parse the five committed validator outputs through closed schema-specific production parsers: canonical pretty JSON for plan, phase, umbrella, and envelope output, and the exact LF-terminated goal PASS line. Reject invalid UTF-8, BOM, CRLF, extra records, stderr, schema drift, and argv/path mismatch. Keep registry path/bytes/SHA external to every child expected stream and semantic parameter so two-pass canonical registry identity remains acyclic. Require exact lowercase `head_commit_oid` and `head_tree_oid` in envelope and registry with byte equality and direct-argv `/usr/bin/git rev-parse --verify HEAD^{commit}` plus `HEAD^{tree}` checks immediately before acceptance and execution. Require 5–12 categorized stop entries with unique closed categories, all five required categories, and exact declared length. Require exact ordered `operation_root`, `registry_root`, `runtime_root`, and `evidence_root` in both envelope and registry with byte equality before capability/destination validation; all role roots are strict, pairwise-disjoint operation-root descendants under lexical, normalized case/separator, realpath, ancestry, reparse, and identity checks. The sole bounded primitive freezes the regular single-link registry and complete operation-to-file ancestor chain before allocation/read, then rechecks exact bytes, SHA, identity, realpath, and relationships. Invoke the sole authority rechecker immediately before every B01–B10 create/open, spawn, publication, cleanup, fsync, and exit effect. Execute only its validated 18-row `repository-diagnostic-command-registry/v1` through the committed bound runner. Validate all 76 create-only destinations, comprising 72 row artifacts and four lifecycle artifacts, strictly beneath `evidence_root`; keep HOME, TAR, streams, temporary state, and cleanup strictly beneath `runtime_root`, and refuse cleanup crossing registry/evidence. The postcommit oracle must execute all 18 safe fake direct-argv children and prove terminal/result-or-failure/cleanup order, runtime absence, registry/evidence preservation, and computed zero recursive deletion from an instrumented exact-identity unlink/bottom-up-rmdir ledger. Require exact 345 runner checks with twenty paired B01–B10 normal/mutant causal cases, all 64 v2 and 11 behavior negatives through full production validation with real isolated topology and bound bytes, three isolated anti-cheat mutants, one measured deletion adapter ledger, and exact 32/32 four-parallel/four-repeat concurrency with preload-denied shared-source writes, native watch evidence, stable identity/hash/mode/time snapshots, and deterministic zero residue. Command IDs grant no authority. Use direct argv, `shell:false`, exact null-prototype environments, raw bounded streams, stop after first failure, and the closed semantic parser. Preserve deterministic primary plus ordered secondary failure truth. Never inspect uncommitted `src/sportsbook/**` or broaden any path, executable, environment, parser, side effect, or destination.
- With `authorityClass: "repository-diagnostic-evidence-set/v1"`, run one initial envelope preflight. It verifies the exact frozen runner and registry paths/bytes/SHA values, closed schemas, and fresh behavioral-execution receipt. Invoke only the registry's allowlisted exact `executable,argv,action,lifecycle` tuples through the fixed runner with direct argv and `shell:false`; command IDs never grant capability. Before each artifact, freeze every existing ancestor from the filesystem root through the artifact parent as exact `{dev,ino,mode,nlink,realpath,type}` non-following identity, reject duplicate-realpath aliases and symlink/reparse ancestry, then re-lstat/re-realpath and exact-compare the full chain immediately before exclusive `wx+`, immediately after open, after file fsync, after same-handle readback/EOF, and immediately before parent-directory fsync. Validate every frozen artifact field by value and relationship, including exact schema/status enums, lowercase64 hashes, booleans, safe integer ranges/counts, nullable fields, paths/identities, event order, `regularNonReparse`, and cleanup outcomes; field-by-field mutation checks must invoke these production validators and assert the expected failure class. No overwrite or rename replacement is allowed. Preserve bounded stdout/stderr and row receipts until terminal plus result/failure publication is durable or its persistence failure is recorded. Cleanup starts only after that publication attempt, acts only on matching non-following identities, refuses substitution/alias/reparse/residue drift, and always records a separate cleanup receipt. The first execution/semantic failure remains primary; persistence and cleanup failures remain ordered secondary errors, while persistence-only or cleanup-only failure becomes primary when no earlier failure exists. Do not rerun the full envelope per artifact.
- Claude, Codex, and OpenCode follow this same capability contract: literal bytes plus the fixed Node runner. Do not use `node -e`, shell interpolation, command substitution, here-doc source reconstruction, generated source, or host-specific wrappers.

All other authority classes or artifact schemas block in preflight. High-risk temporary execution requires the completed five-file `vc-risk-evidence-pack` and explicit APPROVE before DONE.

**Bootstrap exception:** when a phase's own deliverable is the validator script, it cannot run
before it exists. Create it first, run it against the selected plan before any other write,
and record the exception in the phase report.
...
3. Schema-bound completion artifact written: canonical Report for `phase-report/v1`, or exact create-only correction for `phase-report-correction/v1`
