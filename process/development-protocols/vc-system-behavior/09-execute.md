...
- With `authorityClass: "repository-diagnostic-evidence-set/v2"`, run one initial envelope preflight and execute only the validated `repository-diagnostic-command-registry/v1` through the committed bound runner. Validate all 18 structural tuples and all 72 create-only evidence destinations before the first spawn; command IDs grant no authority. Use direct argv, `shell:false`, the exact null-prototype child environment, raw bounded streams, stop after the first failure, and the closed semantic parser selected by each row. Permit only the frozen version, validator, five-path Git, committed-object, and owned-TAR forms. Persist pre/stdout/stderr/post evidence create-only, then terminal plus result/failure before identity-safe cleanup; preserve the first registry, execution, stream, or semantic failure as primary and later persistence/cleanup failures as ordered secondary truth. Never inspect uncommitted `src/sportsbook/**` or broaden any path, executable, environment, parser, side effect, or destination.
