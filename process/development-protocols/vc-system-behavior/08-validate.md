...
`authorityClass: "repository-diagnostic-evidence-set/v2"` selects the additive production diagnostic branch. Produce exact ordered `operation_root`, `registry_root`, `runtime_root`, and `evidence_root` fields. Require each role root to be a strict, pairwise-disjoint operation-root descendant under lexical, normalized case/separator, realpath, ancestry, reparse, and filesystem-identity checks. Bind the regular registry strictly beneath `registry_root`, with 1–1048576 exact bytes, SHA-256, file/root-to-leaf identity and ancestor freezes, and an immediate pre-spawn recheck. Bind a clean committed runner and one canonical 18-row/16-semantic-kind registry. Require all 72 create-only destinations strictly beneath `evidence_root`; require HOME, TAR, streams, temporary state, and cleanup strictly beneath `runtime_root`; cleanup cannot select registry/evidence. IDs are labels only. Only version probes, validators, five-path Git reads, frozen commit/tree reads, and one owned TAR archive are representable; product, network, browser-launch, package, shell/eval, global Git, mutation, ambient-secret, and uncommitted `src/sportsbook/**` access reject before effect.
