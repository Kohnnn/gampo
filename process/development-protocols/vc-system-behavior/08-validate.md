...
## Execution Authority & Evidence Envelope (produced by VALIDATE)

VALIDATE owns production of the envelope. When writing the validate-contract, emit exactly
one fenced block labelled `json execution-authority-envelope/v1` as a
`### Execution Authority & Evidence Envelope` subsection inside `## Validate Contract`, with
exactly six top-level fields:

| field | rule |
|---|---|
| `selected_plan` | repo-relative path to this plan; must exist and be covered by `allowed_scope` |
| `authority_mode` | object with exactly `mode` + `proof_path`; mode ∈ `standing-granted`, `explicit-per-phase` |
| `allowed_scope` | non-empty array of exact paths or single trailing `/**` subtrees; no duplicates, no other globs |
...
| `artifact_schema_version` | closed set: `phase-report/v1`, `phase-report-correction/v1` |

`standing-granted` requires a `proof_path` goal block that passes
`validate-autopilot-goal-block.mjs` and contains the exact line
`EXECUTE CONSENT: standing-granted`. `explicit-per-phase` requires `proof_path` evidence
recording `ENTER EXECUTE MODE` bound to this `selected_plan`.

Destination binding is schema-dependent and closed:

- `phase-report/v1`: `artifact_path` equals exactly one plan line `**Report destination:** \`...\``. Existing report behavior is unchanged.
- `phase-report-correction/v1`: `artifact_path` equals exactly one plan line `**Correction destination:** \`...\``. The correction is a scope-covered, create-only immutable addendum for a previously published Report. A coexisting `**Report destination:**` remains outside artifact binding.
- Every other artifact schema rejects.

Both supported schema identifiers reference their canonical contracts in
[10-update-process.md](10-update-process.md); the envelope does not replace or fork them.

The six-field branch above remains the default whenever `authorityClass` is absent. Preserve its validation/error order and output; apply decoded escaped-equivalent duplicate-key hardening only after selecting the temporary lane. Only work requiring 1–8 exact absolute temporary files may use the isolated `authorityClass: "temporary-artifact-set/v1"` branch. That branch has exactly `selected_plan,authority_mode,authorityClass,allowed_scope,scope_count,stop_conditions,stop_condition_count,artifact_receipt_schema_version`; every scope record has exactly `artifact_path,artifact_schema_version`; counts must match; receipt schema is `execution-temp-artifact-receipt/v1`. Targets must be unique absolute Windows files strictly beneath `C:\Users\Admin\AppData\Local\Temp\opencode`, with no root equality, traversal, globs, UNC/device namespace, prefix collision, trailing dot/space, colon/alternate data stream, case/separator alias, or reserved DOS device basename, including extension-bearing and `COM¹`/`COM²`/`COM³`/`LPT¹`/`LPT²`/`LPT³` aliases. Mixed legacy/temporary fields and unknown authority classes reject. Permission/trust-boundary work requires the five-file `vc-risk-evidence-pack` before completion.

`authorityClass: "repository-diagnostic-evidence-set/v2"` selects the additive production diagnostic branch. Produce exact ordered lowercase `head_commit_oid` and `head_tree_oid` in the envelope and registry, require byte equality, and bind both to direct-argv `/usr/bin/git rev-parse --verify HEAD^{commit}` and `HEAD^{tree}` checks immediately before acceptance and execution. Require 5–12 categorized `stop_conditions`, one unique closed category per entry, all five required categories present, and declared count equal to array length. Produce exact ordered `operation_root`, `registry_root`, `runtime_root`, and `evidence_root` fields and require the registry to declare all four byte-identically before capability or destination validation. Require each role root to be a strict, pairwise-disjoint operation-root descendant under lexical, normalized case/separator, realpath, ancestry, reparse, and filesystem-identity checks. One bounded primitive freezes the regular single-link registry and complete operation-to-file ancestor chain before allocating or reading 1–1048576 exact bytes, then rechecks all identities and relationships. One rechecker guards B01–B10 create/open, spawn, publication, cleanup, fsync, and exit boundaries. Bind a clean committed runner and one canonical 18-row/16-semantic-kind registry. Require all 76 create-only destinations, comprising 72 row artifacts and four lifecycle artifacts, strictly beneath `evidence_root`; require HOME, TAR, streams, temporary state, and cleanup strictly beneath `runtime_root`; cleanup cannot select registry/evidence. The postcommit oracle executes 18 safe fake direct-argv children and requires terminal/result-or-failure/cleanup order, runtime absence, registry/evidence preservation, and computed zero recursive deletion from an instrumented exact-identity unlink/bottom-up-rmdir ledger. Require exact 469 runner checks, including 32 lifecycle checks with twenty paired B01–B10 normal/mutant causal cases, all 64 v2 and 11 behavior negatives through full production validation with real isolated topology and bound bytes, three isolated anti-cheat mutants, one measured deletion adapter ledger, and exact 32/32 four-parallel/four-repeat concurrency with preload-denied shared-source writes, native watch evidence, stable identity/hash/mode/time snapshots, and deterministic zero residue. IDs are labels only. Only version probes, validators, five-path Git reads, frozen commit/tree reads, and one owned TAR archive are representable; product, network, browser-launch, package, shell/eval, global Git, mutation, ambient-secret, and uncommitted `src/sportsbook/**` access reject before effect.

For the v2 diagnostic branch, require exact `469/469`: the retained `449` plus `20/20` runtime-ledger, automatic production cleanup, final cleanup-publication truth, and scoped-index checks. The runtime identity ledger is append-only from actual operation creation, children-first, and excludes registry/evidence/operation roots. Final CLI output is built only after the one create-only cleanup publication attempt. Never inspect the global real index; any optional real-index observation must end with `--` and exactly the twelve authorized harness paths. Complete cached/blob/tree and commit proof uses only an operation-owned `GIT_INDEX_FILE` initialized from verified HEAD.

`authorityClass: "repository-diagnostic-evidence-set/v1"` selects a separate closed repository-local branch with exact ordered envelope, registry, registry-row, scope-record, artifact-receipt, and behavioral-execution-receipt fields. Every registry row is `diagnostic-only`, but names grant no capability: the validator requires the exact canonical registry path, hashes its fenced JSON bytes, compares those bytes with the declared rows, strictly parses the closed `schema,fixture_mode,rows` contract, and permits only allowlisted `executable,argv,action,lifecycle` tuples. A safe operation named `CMD_RM` is allowed; a renamed row with dangerous argv/action/lifecycle is rejected. Every target is unique, normalized, create-only, strictly beneath `evidence_root`, outside product/build roots, executable by a registry row, and collectively covers bootstrap, terminal, result, failure, cleanup, manifest, stdout, stderr, and row-receipt roles plus the four required Phase 02 schemas. Frozen artifact contracts validate every field value and relationship, not key order alone: exact schema/status enums, lowercase64 hashes, booleans, safe integer ranges/counts, nullable fields, paths/identities, event order, `regularNonReparse`, and cleanup outcomes. TAR path gates reject case-insensitive `CON`, `PRN`, `AUX`, `NUL`, `CLOCK$`, `COM1..9`, and `LPT1..9` basename aliases in nested paths, including extensions and trailing spaces/dots; Unicode NFKC-equivalent device aliases such as superscript-digit `COM¹..⁹` and `LPT¹..⁹` also reject while safe near-misses remain accepted. VALIDATE freezes the exact runner path/SHA-256 and requires a closed `repository-diagnostic-behavioral-execution-receipt/v1` whose runner, registry, terminal count, and deterministic terminal SHA match a fresh direct-argv `shell:false` execution. Deleting or mutating that receipt blocks. Source markers, inventories, placeholders, fixture names, hash-only CLI checks, and declared-but-unexecuted behavior are not authority. Claude, Codex, and OpenCode use the same capability contract: literal source/input bytes plus the fixed Node runner, with no shell-generated source or host-specific wrapper.

Temporary PASS uses ordered `status,authorityClass,selected_plan,mode,proof_path,scope_count,stop_condition_count,artifact_receipt_schema_version,artifact_paths` keys. Each path is a deterministic normalized display built from the prechecked approved-root spelling and validated relative components using backslashes; it is not canonical filesystem identity or a recasing claim. Receipt evidence uses ordered `schema,artifactPath,artifactSchemaVersion,bytes,sha256,exclusiveCreate,regularNonReparse,readbackMatches,status` keys and authorizes no additional artifact.

Verify before emitting `PHASE_COMPLETE: VALIDATE`:
...
- `execution-authority-envelope/v1` block written and passing its validator (when the plan uses one)
...
- `### Execution Authority & Evidence Envelope` subsection inside it (when the plan uses one)
- `/goal block` output in chat
