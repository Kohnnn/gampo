...
`authorityClass: "repository-diagnostic-evidence-set/v2"` selects the additive production diagnostic branch. Produce the exact ordered v2 envelope and bind a clean committed runner by normalized absolute path, bytes/SHA-256, blob OID, and commit OID. Bind one variable-path canonical `repository-diagnostic-command-registry/v1` by normalized absolute path, bytes/SHA-256, exact 18 rows, and 72 exact evidence destinations. Validate the complete registry before spawn by executable identity, argv grammar, repository cwd, exact isolated environment, resource/process bounds, one of the 16 closed semantic kinds, frozen live/tree/ignore ledgers, and unique create-only destinations beneath the owned evidence root. IDs are labels only. Only version probes, validators, five-path Git reads, frozen commit/tree reads, and one owned TAR archive are representable; product, network, browser-launch, package, shell/eval, global Git, mutation, ambient-secret, and uncommitted `src/sportsbook/**` access reject before effect.
