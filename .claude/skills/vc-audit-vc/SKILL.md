...
9. Run the execution authority envelope validator against a selected plan:
   ...
   ```
    ...
    `authorityClass: "repository-diagnostic-evidence-set/v2"` selects the additive production diagnostic branch. Its envelope binds a clean committed runner by normalized absolute path, exact bytes/SHA-256, blob OID, and commit OID, and binds one variable-path canonical `repository-diagnostic-command-registry/v1` by normalized absolute path, exact bytes/SHA-256, row count, and exact rows. Validate all 18 rows before spawn using executable identity, argv grammar, repository cwd, exact isolated environment, bounds, process expectations, one of the 16 closed semantic kinds, frozen live/tree/ignore ledgers, and 72 unique create-only evidence destinations. IDs are receipt labels only. Permit only version probes, validators, five-path Git reads, frozen commit/tree reads, and one owned TAR archive; reject product, network, browser-launch, package, shell/eval, global Git, mutation, ambient-secret, and uncommitted `src/sportsbook/**` access before effect.
