...
for (const agent of claudeAgents.filter((name) => codexAgents.includes(name))) {
  ...
  if (normalizedClaude !== normalizedCodex) {
    ...
    // A length delta alone cannot distinguish "one side is missing a whole section"
    // (real drift) from "the same content is worded differently" (acceptable platform
    // divergence). Compute set-difference of trimmed lines so the two are separable:
    // an empty onlyInCodex means Codex is a strict subset of Claude.
    const claudeLines = new Set(normalizedClaude.split("\n").map((l) => l.trim()).filter(Boolean));
    const codexLines = new Set(normalizedCodex.split("\n").map((l) => l.trim()).filter(Boolean));
    const onlyInClaude = [...claudeLines].filter((l) => !codexLines.has(l));
    const onlyInCodex = [...codexLines].filter((l) => !claudeLines.has(l));

    warn(
      `${agent} normalized body differs (${Math.round((lengthDelta / larger) * 100)}% length delta; ` +
        `${onlyInClaude.length} line(s) only in Claude, ${onlyInCodex.length} only in Codex)`
    );

    if (showDiff) {
      diffs.push({
        agent,
        claudeLength: normalizedClaude.length,
        codexLength: normalizedCodex.length,
        onlyInClaude,
        onlyInCodex,
      });
    }
...
if (showDiff) result.diffs = diffs;
