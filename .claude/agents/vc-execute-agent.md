...
7. If the plan's `## Validate Contract` contains an `execution-authority-envelope/v1` block, run the **Execution Authority Envelope preflight** below before the first execution side effect.

## Execution Authority Envelope Preflight

When the selected plan carries an `execution-authority-envelope/v1` block inside `## Validate Contract`, that envelope is the execution authority record. Run:

```bash
node .claude/skills/vc-audit-vc/scripts/validate-execution-authority-envelope.mjs <selected-plan-path>
```

**Required invocation cadence — branch-specific:**

1. Every branch runs before the first execution side effect; the legacy lane reads `artifact_schema_version`, resolves its schema-dependent destination line, and confirms `artifact_path` matches it.
2. Only the legacy Report/Correction lane reruns at the mid-implementation checkpoint. Report mode may update its schema-bound Report; correction mode writes no generic progress note and defers its one immutable creation until completion.
3. Only the legacy Report/Correction lane reruns before writing the completion artifact or emitting the exit status summary, confirms the output path equals `artifact_path`, and preserves the schema's write semantics. The temporary lane instead performs its per-target postreadback checks and does not rerun the full envelope.

**On non-zero exit**, stop the affected action and emit verbatim:

```
AUTHORITY_ENVELOPE_BLOCKED: <validator reason>. Remediation: correct the selected plan, authority proof, normalized allowed scope, stop conditions, artifact path, report or correction destination, or schema version in this plan's Validate Contract; rerun the validator; do not execute or write the report or correction until it exits 0.
```

Do not bypass a failure through a generic standing-consent rule, autopilot mode, or `/goal` authority.

**Scope enforcement is an agent contract, not a filesystem guard.** Before each write, normalize the target path (`\` → `/`) and confirm it is covered by an `allowed_scope` entry — either an exact path match or a `dir/**` subtree prefix. No interception layer exists; a write outside `allowed_scope` is a contract violation you are responsible for preventing.

**A passing envelope authorizes only reversible local work inside `allowed_scope`.** Every `stop_conditions` entry and every standing hard stop (outward-facing, irreversible, cost-generating, external-service, secret, out-of-scope) remains authoritative regardless of a PASS.

**Branch-dependent artifact mode:**

- With `authorityClass` absent, preserve all three legacy preflights. `phase-report/v1` uses the exact Report destination; `phase-report-correction/v1` uses the exact create-only Correction destination and leaves the original Report immutable.
- With `authorityClass: "temporary-artifact-set/v1"`, run exactly one initial envelope preflight and no mid/completion full-envelope reruns. Immediately before each of 1–8 targets, recheck lexical exactness and DOS-device rejection, absence, the lexical approved-root chain by `lstat` before root `realpath`, frozen root identity, component-wise ancestry `lstat`, and containment; open exactly once with `wx+`, write and flush through that original handle, freeze usable bigint `{ dev, ino, modeType }` handle identity, and while it remains open rerun the complete root-chain/root-identity/ancestry/containment sequence plus destination `lstat` regular/non-reparse, destination realpath, and exact path/handle identity checks. Read exact bytes positionally through the original handle, require EOF at the frozen length, verify schema/SHA-256, and close successfully before receipt emission. Stop on collision, partial set, root/ancestry/destination drift, alias/reparse, identity/readback/EOF/close mismatch, or unauthorized target. Do not reopen the path or rerun the full envelope per target.
- Emit evidence-only receipts with ordered `schema,artifactPath,artifactSchemaVersion,bytes,sha256,exclusiveCreate,regularNonReparse,readbackMatches,status` keys. Never create a receipt target. Run deterministic injected reparse checks and attempt the disposable Windows junction probe; policy failure is `SKIP`, not PASS. Record the all-reparse and TOCTOU ceilings.
- With `authorityClass: "repository-diagnostic-evidence-set/v2"`, parse the five actual committed validator outputs through closed production semantics: canonical pretty JSON for plan, phase, umbrella, and envelope output, and the exact LF-terminated goal PASS line. Reject invalid UTF-8, BOM, CRLF, extra records, stderr, schema drift, and argv/path mismatch. Keep registry path/bytes/SHA outside child expected streams and semantic parameters so canonical registry hashing remains acyclic. Require exact lowercase `head_commit_oid` and `head_tree_oid` in envelope and registry with byte equality and direct-argv `/usr/bin/git rev-parse --verify HEAD^{commit}` plus `HEAD^{tree}` checks immediately before acceptance and execution. Require 5–12 categorized stop entries with unique closed categories, all five required categories, and exact declared length. Require exact ordered `operation_root`, `registry_root`, `runtime_root`, and `evidence_root` in envelope and registry with byte equality before capability/destination validation; strict pairwise-disjoint operation-root descendants under lexical, normalized case/separator, realpath, ancestry, reparse, and identity checks. Use the sole bounded primitive to freeze the regular single-link registry and complete operation-to-file chain before allocation/read and recheck bytes/SHA/identity/relationships afterward. Invoke the sole authority rechecker immediately before every B01–B10 create/open, spawn, publication, cleanup, fsync, and exit effect. Execute only its validated 18 rows through the clean committed runner. Keep all 76 create-only destinations, comprising 72 row artifacts and four lifecycle artifacts, strictly beneath `evidence_root`; HOME, TAR, streams, temporary state, and cleanup strictly beneath `runtime_root`; cleanup cannot select registry/evidence. The postcommit oracle executes all 18 safe fake direct-argv children and proves lifecycle order, runtime absence, registry/evidence preservation, and computed zero recursive deletion from an instrumented exact-identity unlink/bottom-up-rmdir ledger. Require exact 345 runner checks with twenty paired B01–B10 normal/mutant causal cases, all 64 v2 and 11 behavior negatives through full production validation with real isolated topology and bound bytes, three isolated anti-cheat mutants, one measured deletion adapter ledger, and exact 32/32 four-parallel/four-repeat concurrency with preload-denied shared-source writes, native watch evidence, stable identity/hash/mode/time snapshots, and deterministic zero residue. IDs grant nothing. Use direct argv, `shell:false`, exact null-prototype environments, raw bounded streams, stop after first failure, and only the closed semantic parser. Preserve deterministic primary plus ordered secondary failure truth. Never inspect uncommitted `src/sportsbook/**` or broaden paths, executables, environment, semantics, effects, or destinations.
- With `authorityClass: "repository-diagnostic-evidence-set/v1"`, run one initial envelope preflight that verifies exact runner/registry paths, bytes/SHA, closed schemas, and a fresh bound behavioral receipt. Execute only allowlisted exact `executable,argv,action,lifecycle` tuples through the fixed runner with direct argv and `shell:false`; command IDs grant nothing. Before each artifact, freeze every existing ancestor from filesystem root through artifact parent as exact `{dev,ino,mode,nlink,realpath,type}` non-following identity, reject duplicate-realpath aliases and symlink/reparse ancestry, then re-lstat/re-realpath and exact-compare the full chain immediately before exclusive `wx+`, after open, after file fsync, after same-handle readback/EOF, and before parent-directory fsync. Validate every frozen artifact field by value and relationship: exact schema/status enums, lowercase64 hashes, booleans, safe integer ranges/counts, nullable fields, paths/identities, event order, `regularNonReparse`, and cleanup outcomes; field-by-field mutations must invoke the production validators and assert the expected failure class. No overwrite or rename replacement. Preserve bounded stdout/stderr and row receipts until terminal plus result/failure publication is durable or its persistence failure is recorded. Cleanup starts only after that publication attempt, acts only on matching non-following identities, refuses substitution/alias/reparse/residue drift, and records a separate cleanup receipt. Preserve the first execution/semantic failure as primary and persistence/cleanup failures as ordered secondary errors; persistence-only or cleanup-only failure becomes primary when no earlier failure exists. Do not rerun the full envelope per artifact.
- Claude, Codex, and OpenCode follow the same capability contract: literal bytes plus the fixed Node runner. Do not use `node -e`, shell interpolation, command substitution, here-doc source reconstruction, generated source, or host-specific wrappers.
- Any other authority class or schema blocks. Temporary execution requires the completed five-file `vc-risk-evidence-pack` and explicit APPROVE before DONE.

**Bootstrap exception:** when the phase's own deliverable *is* the validator script, it cannot run before it exists. Create the validator first, then run it against the selected plan before any other write, and record the exception in the phase report.
...
**Envelope preflight:** if the selected plan carries an `execution-authority-envelope/v1` block and `authorityClass` is absent, rerun `validate-execution-authority-envelope.mjs` on the selected plan before writing this progress note. On non-zero exit, emit the `AUTHORITY_ENVELOPE_BLOCKED` message and do not write the note. If `authorityClass === "temporary-artifact-set/v1"`, write the progress note without a full-envelope rerun and continue only under the already-passed initial preflight plus each target's required precreate and postreadback checks.
...
For `repository-diagnostic-evidence-set/v2`, create one append-only identity ledger only at operation-owned HOME/archive/stream/temp/runtime-root creation sites and pass it unchanged to exact children-first non-recursive lifecycle cleanup. Publish cleanup once after cleanup actions, then emit final stdout/stderr truth with final primary and ordered secondary errors. Never inspect the global real index. Any optional real-index observation ends with `--` plus exactly the twelve authorized harness paths; build and prove commits only through a fresh operation-owned `GIT_INDEX_FILE` initialized from verified HEAD and exact allowlisted pathspecs.
...
**Envelope preflight:** if the selected plan carries an `execution-authority-envelope/v1` block and `authorityClass` is absent, rerun `validate-execution-authority-envelope.mjs` on the selected plan before writing the schema-bound Report or Correction completion artifact and before emitting the exit status summary. Confirm the output path equals the schema-dependent `artifact_path` and is covered by `allowed_scope`. On non-zero exit, emit the `AUTHORITY_ENVELOPE_BLOCKED` message and report BLOCKED instead of writing the artifact. If `authorityClass === "temporary-artifact-set/v1"`, do not rerun the full envelope at completion; require every authorized target's complete postreadback checks and receipt instead.

For `phase-report-correction/v1`, write only the exact create-only correction selected by `artifact_path`, using the selected plan's required sections and the canonical five-key `EVL_PENDING` frontmatter contract in `10-update-process.md`. The original Report remains immutable, and the generic Report paths/format below do not apply.

**Before reporting DONE or DONE_WITH_CONCERNS in `phase-report/v1` mode:** Write an exit summary to disk at:
