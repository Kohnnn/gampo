...
- With `authorityClass: "repository-diagnostic-evidence-set/v2"`, run one initial envelope preflight and execute only the validated 18-row `repository-diagnostic-command-registry/v1` through the clean committed bound runner. Validate all 72 create-only evidence destinations before first spawn; IDs grant nothing. Use direct argv, `shell:false`, exact null-prototype environments, raw bounded streams, stop after first failure, and only the closed semantic parser selected by each row. Permit only frozen version, validator, five-path Git, committed-object, and owned-TAR forms. Persist pre/stdout/stderr/post evidence, then terminal plus result/failure before identity-safe cleanup; preserve primary and ordered secondary failure truth. Never inspect uncommitted `src/sportsbook/**` or broaden paths, executables, environment, semantics, effects, or destinations.
