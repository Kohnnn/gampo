...
- With `authorityClass: "repository-diagnostic-evidence-set/v2"`, require exact ordered `operation_root`, `registry_root`, `runtime_root`, and `evidence_root`; strict pairwise-disjoint operation-root descendants under lexical, normalized case/separator, realpath, ancestry, reparse, and identity checks. Keep the exact bounded/hash/identity/ancestor-frozen registry strictly beneath `registry_root` through the immediate pre-spawn recheck. Execute only its validated 18 rows through the clean committed runner. Keep all 72 create-only destinations strictly beneath `evidence_root`; HOME, TAR, streams, temporary state, and cleanup strictly beneath `runtime_root`; cleanup cannot select registry/evidence. IDs grant nothing. Use direct argv, `shell:false`, exact null-prototype environments, raw bounded streams, stop after first failure, and only the closed semantic parser. Preserve registry/evidence through runtime cleanup and preserve primary plus ordered secondary failure truth. Never inspect uncommitted `src/sportsbook/**` or broaden paths, executables, environment, semantics, effects, or destinations.
