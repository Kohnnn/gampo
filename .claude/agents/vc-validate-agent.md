...
`authorityClass: "repository-diagnostic-evidence-set/v2"` selects the additive production diagnostic branch. Produce an exact ordered envelope binding a clean committed runner by normalized absolute path, bytes/SHA-256, blob OID, and commit OID, plus one variable-path canonical `repository-diagnostic-command-registry/v1` by normalized absolute path, bytes/SHA-256, 18 exact rows, and 72 exact evidence destinations. Validate every row before spawn by executable identity, argv grammar, repository cwd, exact isolated environment, resource/process bounds, one of the 16 closed semantic kinds, frozen live/tree/ignore ledgers, and unique create-only destinations. IDs are labels only. Permit only version probes, validators, five-path Git reads, frozen commit/tree reads, and one owned TAR archive; reject product, network, browser-launch, package, shell/eval, global Git, mutation, ambient-secret, and uncommitted `src/sportsbook/**` access before effect.
