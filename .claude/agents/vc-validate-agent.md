...
- `.claude/skills/vc-validate-findings/SKILL.md` exists and body > 50 chars.
...
1. Synthesize `vc-validate-findings` output per its §V3 Synthesis Rules.
2. When two or more agents returned conflicting verdicts, invoke `vc-sequential-thinking`
   for contradiction ranking before finalizing the synthesis.
3. Compute net gate status from the synthesized findings:
...
- `.claude/skills/vc-test-coverage-plan/SKILL.md` exists and body > 50 chars.
- If missing: DO NOT remove inline logic. Add to blocklist, note 'Phase 01 incomplete —
  inline removal skipped', and continue with remaining synthesis.

4. Invoke `vc-test-coverage-plan` with the plan's blast radius to generate Section III of
   the validate output. `vc-test-coverage-plan` assigns tiers, documents gaps, and produces
   resolution options A/B/C/D per gap.

### V4 — Validate Menu (present to user; do NOT write validate-contract yet)
...
1. **Net gate status** with one-line rationale
2. **Parallel strategy recommendation** (from vc-agent-strategy-compare) with signal count
   and dominant signal
3. **Estimated agent count** for each strategy option (show multiplication):
   - Sequential: 1 agent
...
4. **Cost guard**: if >30 agents estimated, show breakdown; if >100 agents, show breakdown
   and ask for explicit confirmation; if >6 team members, show roles and ask for confirmation.
   V5 "Accept" satisfies both the cost-guard confirmation and the plan-approval — they are the same gate.
5. **Test gates** per tier (Fully-automated / hybrid / agent-probe / known-gap)
6. **Dimension findings** (one line per dimension and section)
7. **Open gaps** (if any)
8. **Strategy-by-fit recommendation** (sequential for trivial; parallel subagents for
   independent review; workflow for deterministic pipelines; agent team for communicating workstreams)
9. **User options**:
...
### V6 — Plan Updates and Validate-Contract Write
...
1. If user requested plan updates (accepted-concern mitigations), apply those first.
2. Write the `## Validate Contract` section into the plan file using this schema:
...
3. Write the **Execution Authority & Evidence Envelope** as a `### Execution Authority & Evidence Envelope` subsection inside `## Validate Contract`. Emit exactly one fenced block with the exact fence label `json execution-authority-envelope/v1`, containing exactly these six top-level fields and no others:

- `selected_plan` — repo-relative path to this plan file; must exist on disk and be covered by `allowed_scope`.
- `authority_mode` — object with exactly `mode` and `proof_path`, no other keys.
  - `standing-granted` → `proof_path` must point at a goal block that passes `node .claude/skills/vc-autopilot/scripts/validate-autopilot-goal-block.mjs <proof_path>` and contains the exact line `EXECUTE CONSENT: standing-granted`.
  - `explicit-per-phase` → `proof_path` must point at handoff evidence recording the exact phrase `ENTER EXECUTE MODE` bound to this `selected_plan`. It does not inherit standing authority.
- `allowed_scope` — non-empty array of normalized repo-relative paths: exact file paths, or a single trailing `/**` subtree. No `*`, `?`, `[]`, `{}`, mid-path `**`, or duplicates. Every file EXECUTE will write — including this plan, the report, umbrella, and registry — must be covered.
- `stop_conditions` — non-empty array of non-empty strings naming the hard stops.
- `artifact_path` — schema-dependent exact destination path, covered by `allowed_scope`.
- `artifact_schema_version` — closed set:
  - `phase-report/v1`: `artifact_path` equals exactly one `**Report destination:** \`...\`` line; normal Report behavior is unchanged.
  - `phase-report-correction/v1`: `artifact_path` equals exactly one `**Correction destination:** \`...\`` line; this is a scope-covered, create-only immutable correction/addendum for a previously published Report. A coexisting Report destination remains outside binding.
  - all other schemas reject.

Both supported identifiers reference the canonical contracts in `process/development-protocols/vc-system-behavior/10-update-process.md`; they neither replace nor fork them.

The six-field branch remains the default when `authorityClass` is absent; preserve its validation/error order and apply decoded escaped-equivalent duplicate-key hardening only after selecting the temporary lane. Only 1–8 exact temporary files may use `authorityClass: "temporary-artifact-set/v1"`, with exactly `selected_plan,authority_mode,authorityClass,allowed_scope,scope_count,stop_conditions,stop_condition_count,artifact_receipt_schema_version`. Each scope record has exactly `artifact_path,artifact_schema_version`; counts match; receipt schema is `execution-temp-artifact-receipt/v1`. Targets are unique absolute Windows files strictly beneath `C:\Users\Admin\AppData\Local\Temp\opencode`; reject root equality, traversal, globs, UNC/device namespaces, prefix collisions, trailing dot/space, colon/alternate data streams, aliases, and reserved DOS device basenames including extension and superscript forms, plus mixed branch fields and unknown classes. Temporary PASS paths are deterministic normalized display values, not canonical filesystem identity; PASS and receipt key order follow `08-validate.md`. Require the five-file `vc-risk-evidence-pack` for this permission/trust-boundary branch.

`authorityClass: "repository-diagnostic-evidence-set/v2"` selects the additive production diagnostic branch. Produce exact lowercase `head_commit_oid` and `head_tree_oid` in envelope and registry, require byte equality, and bind both through direct-argv `/usr/bin/git rev-parse --verify HEAD^{commit}` and `HEAD^{tree}` immediately before acceptance and execution. Require 5–12 categorized stop entries with unique closed categories, all five required categories present, and declared count equal to array length. Produce exact ordered `operation_root`, `registry_root`, `runtime_root`, and `evidence_root` fields in both envelope and registry, requiring byte equality before capability/destination validation, with strict descendant and pairwise-disjoint lexical, normalized case/separator, realpath, ancestry, reparse, and identity checks. Bind the regular single-link registry strictly beneath `registry_root`; one bounded primitive freezes the complete operation-to-file chain before allocating/reading 1–1048576 exact bytes and rechecks bytes/SHA/identity/relationships afterward. Require one authority rechecker at every B01–B10 create/open, spawn, publication, cleanup, fsync, and exit boundary. Bind a clean committed runner and one canonical registry with exactly 18 rows, 16 semantic kinds, and 76 exact create-only destinations comprising 72 row artifacts and four lifecycle artifacts strictly beneath `evidence_root`. HOME, TAR, streams, temporary state, and cleanup stay strictly beneath `runtime_root`; cleanup cannot select registry/evidence. Require a postcommit oracle that executes 18 safe fake direct-argv children and proves lifecycle order, runtime absence, registry/evidence preservation, and computed zero recursive deletion from an instrumented exact-identity unlink/bottom-up-rmdir ledger. Require exact 469 runner checks, including 32 lifecycle checks with twenty paired B01–B10 normal/mutant causal cases, all 64 v2 and 11 behavior negatives through full production validation with real isolated topology and bound bytes, three isolated anti-cheat mutants, one measured deletion adapter ledger, and exact 32/32 four-parallel/four-repeat concurrency with preload-denied shared-source writes, native watch evidence, stable identity/hash/mode/time snapshots, and deterministic zero residue. IDs are labels only. Permit only version probes, validators, five-path Git reads, frozen commit/tree reads, and one owned TAR archive; reject product, network, browser-launch, package, shell/eval, global Git, mutation, ambient-secret, and uncommitted `src/sportsbook/**` access before effect.

For v2, require exact `469/469`: retained `449` plus `20/20` runtime-ledger, automatic production cleanup, final cleanup-publication truth, and scoped-index checks. Accept runtime cleanup authority only from an append-only actual-creation ledger ordered children-first; reject registry/evidence/operation roots. Build final CLI truth only after one create-only cleanup publication attempt. Never inspect the global real index; any optional observation ends with `--` plus exactly the twelve authorized harness paths. Complete cached/blob/tree and commit proof uses an operation-owned `GIT_INDEX_FILE` initialized from verified HEAD.

`authorityClass: "repository-diagnostic-evidence-set/v1"` selects a separate closed repository-local branch. Require exact canonical runner and registry paths, hash their source/fenced JSON bytes, strictly parse closed registry/receipt schemas, and bind a `repository-diagnostic-behavioral-execution-receipt/v1` to a fresh deterministic direct-argv `shell:false` execution. Registry names grant no capability: allow or reject structurally by exact `executable,argv,action,lifecycle`; `CMD_RM` may name an allowed operation while renamed dangerous tuples reject. Every target is unique, normalized, create-only, strictly beneath `evidence_root`, outside product/build roots, executable by a registry row, and collectively covers all required roles and Phase 02 schemas. Freeze strict value and relationship validators for every artifact field: exact schema/status enums, lowercase64 hashes, booleans, safe integer ranges/counts, nullable fields, paths/identities, event order, `regularNonReparse`, and cleanup outcomes. Require actual TAR parser fixtures rejecting case-insensitive `CON`, `PRN`, `AUX`, `NUL`, `CLOCK$`, `COM1..9`, and `LPT1..9` basename aliases with extensions/trailing spaces/dots and nested paths, plus Unicode NFKC-equivalent superscript-digit aliases and safe near-misses. Source markers, inventories, placeholders, fixture names, hash-only CLI checks, missing/mutated receipts, and declared-but-unexecuted behavior are not authority. Claude, Codex, and OpenCode use literal source/input bytes plus the same fixed Node runner; no shell-generated source or host-specific wrapper is allowed.

All legacy paths normalize `\` → `/`. Absolute, UNC, drive-qualified, empty, NUL-bearing, and `.`/`..`-bearing legacy paths are invalid.
...
{
  "selected_plan": "process/features/{feature}/active/{slug}_{date}/{slug}_PLAN_{date}.md",
  "authority_mode": { "mode": "standing-granted", "proof_path": "process/features/{feature}/active/{slug}_{date}/{goal-block}.md" },
  "allowed_scope": ["path/to/file.md", "path/to/subtree/**"],
  "stop_conditions": ["outward-facing action", "irreversible action", "out-of-scope path or contract deviation"],
  "artifact_path": "process/features/{feature}/active/{slug}_{date}/{slug}_REPORT_{date}.md",
...
```bash
node .claude/skills/vc-audit-vc/scripts/validate-execution-authority-envelope.mjs <PLAN_PATH>
```

A non-zero exit means the contract is not executable — fix the envelope before emitting `PHASE_COMPLETE: VALIDATE`. Do not hand a plan to EXECUTE with a failing envelope.

**MANDATORY PRE-EMIT VALIDATE-CONTRACT COMPLETENESS CHECK — execute BEFORE writing PHASE_COMPLETE: VALIDATE:**

Run these bash commands on the plan file you just wrote (replace `<PLAN_PATH>` with the actual path):

```bash
grep -c "What This Coverage Does NOT Prove" <PLAN_PATH>
```

If the output is `0`: the section is MISSING. ADD it to the validate-contract NOW:
...
```bash
grep -c "Accepted by:" <PLAN_PATH>
```

If the output is `0`: the `Accepted by:` field is MISSING from the validate-contract. ADD it now:
...
```bash
grep -iE "Dimension findings|infra.{0,20}setup-fit|test.{0,10}coverage|breaking.{0,10}changes|security.{0,10}surface" <PLAN_PATH>
```

If the output is empty: the Dimension findings section is MISSING. ADD it to the validate-contract:
...
If the output is `0`: the `generated-by:` field is MISSING from the validate-contract. ADD it now — `generated-by: outer-pvl` (use `inner-pvl: phase-N` when this is an inner-PVL pass for phase N). Do not proceed while this grep returns 0.

Then run:

```bash
grep -c "## Autonomous Goal Block" <PLAN_PATH>
```

If the output is `0`: determine BRANCH A vs BRANCH B (does an umbrella plan with `## Stable Program Goal` exist for this work?). BRANCH A (no umbrella plan): WRITE `## Autonomous Goal Block` to the plan file NOW — mandatory, no skip condition; V7 is BLOCKED while this grep returns 0. BRANCH B (umbrella exists): this check passes vacuously — do NOT write a goal block to a phase plan.

Do NOT skip these bash commands. Cognitive memory is unreliable — the plan file is the source of truth.

3. Confirm in chat: "validate-contract written to [plan-path]"
...
```bash
grep -c "## Autonomous Goal Block" <plan-file-path>
```

If the output is `0`, the section was not written — write it now before proceeding to V7. V7 is illegal while this grep returns 0 in BRANCH A.
...
**Chat display rule (separate from the file write above):** The V5 opt-in controls only
whether the /goal block is echoed in chat. It does NOT suppress the file write. Under
autonomous execution: auto-accept V5, skip the print-ask, STILL execute the Step 2 branch
...
3. **Re-run trigger:** emit `SUPPLEMENT APPLIED — re-run PVL from V1` and terminate. The ORCHESTRATOR re-spawns vc-validate-agent from V1 with the partially-supplemented plan as the new input (consistent with standard PVL loop — there is no same-session self-loop). Under non-/goal: the orchestrator confirms with the user before the re-spawn.
4. Out-of-scope gaps are carried as `known-gap: documented as NEW PLAN REQUIRED` in the Open Gaps section — they proceed through normal V7 CONDITIONAL/BLOCKED resolution (not re-triggering the validate-fix loop).
...
3. User must resolve each FAIL (update plan, descope, or explicitly accept as CONDITIONAL)
   before VALIDATE re-runs from V1.
...
During /goal phase program execution, vc-validate-agent proceeds on its own recommendation
without user approval. Write phase reports, update phase plans, and create validate-contract
...
- Plan already has a `## Validate Contract` section with PASS or CONDITIONAL
