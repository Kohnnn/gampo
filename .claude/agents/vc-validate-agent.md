...
`authorityClass: "repository-diagnostic-evidence-set/v2"` selects the additive production diagnostic branch. Produce exact ordered `operation_root`, `registry_root`, `runtime_root`, and `evidence_root` fields with strict descendant and pairwise-disjoint lexical, normalized case/separator, realpath, ancestry, reparse, and identity checks. Bind the regular registry strictly beneath `registry_root`, bounded to 1–1048576 exact bytes, SHA/identity/ancestor-frozen, and rechecked immediately before first spawn. Bind a clean committed runner and one canonical registry with exactly 18 rows, 16 semantic kinds, and 72 exact create-only destinations strictly beneath `evidence_root`. HOME, TAR, streams, temporary state, and cleanup stay strictly beneath `runtime_root`; cleanup cannot select registry/evidence. IDs are labels only. Permit only version probes, validators, five-path Git reads, frozen commit/tree reads, and one owned TAR archive; reject product, network, browser-launch, package, shell/eval, global Git, mutation, ambient-secret, and uncommitted `src/sportsbook/**` access before effect.
