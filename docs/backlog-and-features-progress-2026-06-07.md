...
## Follow-on shipped (WCAG contrast, 2026-06-07)
- Reinforce #7 DONE: built `scripts/auditContrast.mjs` (+`npm run audit:contrast`) — real WCAG 2.1 ratio computation per visible text node, compositing translucent fg/bg through ancestors, gradient-aware (skips text on gradient/image backgrounds to avoid false positives), AA thresholds (4.5 normal / 3.0 large-bold).
